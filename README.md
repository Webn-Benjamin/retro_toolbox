...
[![Version](https://img.shields.io/badge/version-1.2.1-orange?style=flat-square)](https://retro-toolbox.fr)
[![Platform](https://img.shields.io/badge/platform-Windows%20%7C%20macOS-blue?style=flat-square)](https://retro-toolbox.fr)
...
| 💰 **Prix HDV** | Prix de l'Hôtel de Vente partagés par la communauté — par serveur, par lot ×1/×10/×100, historique avec graphique et favoris |
| 💎 **Runes** | Tableau de poids des runes, calculateur de puit, estimation d'obtention PA/PM et compteur de tentatives exo |
| 📝 **Notes** | Plusieurs notes distinctes avec mise en forme — gras, couleurs, tailles, cases à cocher |
| 🧮 **Calculateurs** | Calculateur d'XP métier et calculateur Craft / HDV regroupés dans un seul onglet |
...
| 🕐 **Session** | Chrono de farm, chrono donjon cumulatif, compteur de donjons et de captures, récapitulatif automatique |
| ⚙ **Paramètres** | Dossier de données, seuil d'alerte timer en minutes, thème clair/sombre |
| 📊 **Détails** | À propos — version et lien Discord |

---

### Onglet Prix HDV — détail

Les prix de l'Hôtel de Vente sont partagés entre les joueurs et hébergés sur un backend communautaire. Aucun compte n'est requis — seulement un pseudo.

| Feature | Description |
|---------|-------------|
| 🌐 **Prix par serveur** | Consultez et proposez des prix par serveur Dofus Rétro (Boune, Allisteria, Fallanster) |
| 📦 **Prix par lot** | Trois lots par ressource : ×1, ×10 et ×100. Le ×1 est obligatoire, les autres optionnels |
| 📈 **Historique** | Graphique d'évolution des prix par lot sur 7, 15 ou 30 jours |
| ⭐ **Favoris** | Épinglez vos ressources suivies dans un panneau dédié, avec leurs prix |
| 🔍 **Recherche & tri** | Recherche d'item, filtres par catégorie et tri (récent, nom, prix croissant/décroissant) |
| ✎ **Modifier** | Mettez à jour un prix directement depuis sa carte, lots pré-remplis |
| 🔒 **Anti-troll** | Validation statistique par médiane : les prix aberrants sont automatiquement rejetés |
| ⚑ **Signalement** | Un prix suspect peut être signalé ; masqué automatiquement au-delà de plusieurs signalements |
| 👤 **Pseudos uniques** | Chaque pseudo est unique (insensible à la casse) pour éviter l'usurpation |
| ⏱ **Limites anti-spam** | Délais entre soumissions (création / modification) par installation et par IP |
...
### Onglet Calculateurs — détail

| Feature | Description |
|---------|-------------|
| 🧮 **Calculateur XP métier** | Nombre de crafts et de parchemins nécessaires pour monter un métier d'un niveau à un autre, avec multiplicateur d'XP et tableau détaillé des paliers |
| 🔧 **Calculateur Craft / HDV** | Coût des ressources, prix hôtel de vente, marge en temps réel et OCR clic droit |

---

### Onglet Craft / HDV — détail
...
| 🏰 **Chrono donjon cumulatif** | Temps total passé en donjon, cumulé sur la session |
| 🔢 **Compteur de donjons** | Incrémentez / décrémentez le nombre de donjons effectués avec boutons + / − et reset |
| 🎯 **Compteur de captures** | Suivi du nombre de captures réalisées pendant la session |
| 📊 **Récapitulatif automatique** | Calcule en temps réel les donjons par heure, le temps moyen par donjon et les captures par heure |
| 💾 **Persistance** | Le chrono et les compteurs sont sauvegardés et restaurés à chaque ouverture |

---

## Prérequis

- Windows 10 / 11 (support macOS en bêta)
...
> **Détection OCR clic droit** (Craft / HDV) : nécessite **Tesseract OCR** installé séparément.
...
├── hdv_prices.py              # Client API des prix HDV communautaires (Supabase)
...
│   ├── hdv_tab.py             # Onglet prix HDV communautaires
│   ├── runes_tab.py           # Onglet runes / puit / calcul PA-PM / exo
│   ├── todo_tab.py            # Onglet notes
│   ├── calculators_tab.py     # Onglet calculateurs (menu XP métier + Craft/HDV)
│   ├── xp_metier_tab.py       # Calculateur d'XP métier
│   ├── craft_hdv_tab.py       # Calculateur Craft / HDV
│   ├── craft_tab.py           # Logique craft / OCR clic droit
│   ├── timer_tab.py           # Onglet timer respawn
│   ├── challenges_tab.py      # Onglet challenges / sorts
│   ├── overlay_tab.py         # Onglet dots / overlay
│   ├── session_tab.py         # Onglet session / chrono / donjons / captures
...
│       ├── sadi_manager.py    # Logique du mode Farm Sadi
│       ├── characters_panel.py
│       ├── shortcuts_panel.py
│       ├── settings_panel.py
│       ├── farmsadi_panel.py
...
L'application sauvegarde vos réglages **localement** en JSON ; aucune donnée personnelle n'est envoyée sur internet.

La feature **Prix HDV** communique avec un backend communautaire (Supabase) uniquement pour partager et consulter des prix. Aucun compte, aucune adresse e-mail : votre identité est un identifiant anonyme local et un pseudo de votre choix. Les adresses IP ne sont jamais stockées en clair (uniquement sous forme de hash) et servent seulement aux limites anti-spam.

La fonctionnalité OCR (Craft / HDV) effectue un screenshot localement uniquement lors d'un clic droit sur Dofus, quand l'onglet concerné est actif.

---

## Faux positifs antivirus

Les exécutables compilés avec PyInstaller peuvent être signalés à tort par certains antivirus. Retro Toolbox ne lit pas l'écran en permanence, n'accède pas à vos fichiers personnels et ne se connecte à aucun serveur tiers (hormis la vérification de mises à jour sur `retro-toolbox.fr` et le backend communautaire des prix HDV).
