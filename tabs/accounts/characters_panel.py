...
from tabs.accounts.focus_guard import FocusGuard
...
class AccountPanel(QWidget):
    ...
    def _build(self):
        ...
        # Bouton Supprimer (Appliquer retiré : la sélection applique déjà directement)
        btn_del_prof = QPushButton("✕  Supprimer")
        btn_del_prof.setFixedHeight(30)
        btn_del_prof.setStyleSheet(
            f"QPushButton{{background:{T.BG_DARK};color:{T.RED};border:1px solid {T.BORDER};"
            f"padding:4px 10px;font-size:9pt;font-weight:700;border-radius:8px;}}"
            f"QPushButton:hover{{background:{T.RED};color:white;border:none;}}")
        btn_del_prof.setCursor(Qt.CursorShape.PointingHandCursor)
        pl.addWidget(btn_del_prof)
        ...
        # La sélection dans le combo applique directement l'ordre du profil
    ...
    def _discover(self):
        ...
        # Tenir le FocusGuard à jour (liste des hwnds et pseudos exclus)
        self._focus_guard.update(
            self._excluded,
            {w.hwnd: w.pseudo for w in self._windows},
        )
        self._maybe_apply_active_profile()
        self._render()

    def _maybe_apply_active_profile(self):
        """Réordonne automatiquement selon le profil actif dès que TOUS ses
        comptes sont connectés — utile au démarrage quand Dofus met du temps
        à ouvrir toutes les fenêtres, ou si elles se connectent progressivement.
        Ne se déclenche qu'une fois par session pour ne pas réordonner sans
        arrêt si l'utilisateur réorganise manuellement ensuite."""
        if getattr(self, "_profile_applied_this_session", True):
            return
        active = self._cfg.get("active_profile", "")
        if not active or active not in self._profiles:
            return
        pseudos = self._profiles[active]
        if not pseudos:
            return
        connected = {w.pseudo for w in self._windows if not w.loading}
        if not all(p in connected for p in pseudos):
            return  # pas encore tous connectés, on réessaiera au prochain scan
        ...
        self._profile_applied_this_session = True
        self._status.setText(f"✅ Profil «{active}» appliqué automatiquement")
    ...
    def _on_profile_selected(self, name: str):
        """Sélectionner un profil l'applique immédiatement (réordonne les
        fenêtres), sans avoir besoin d'un bouton "Appliquer" séparé."""
        if name == "aucun profil":
            self._cfg["active_profile"] = ""
            self._profile_applied_this_session = True
            self._on_save()
            self._status.setText("Aucun profil sélectionné")
            return
        ...
        # Appliquer l'ordre dans la barre des tâches sans écraser le status
        hwnds = [w.hwnd for w in self._windows if w.hwnd]
        if hwnds:
            import threading
            threading.Thread(target=self._do_reorder, args=(hwnds,), daemon=True).start()
    ...
    def _delete_profile(self):
        ...
        self._cfg["order_profiles"] = self._profiles
        # Si on supprime le profil actif, revenir à "aucun profil"
    ...
    def _start_poll(self):
        # Au lancement : toujours repartir sur "aucun profil" —
        # l'utilisateur choisit lui-même le profil à appliquer.
        self._cfg["active_profile"] = ""
        self._profile_applied_this_session = True  # bloquer l'auto-apply
        self._refresh_profile_combo()              # forcer combo sur index 0
        self._discover()
    ...
    def cycle(self, direction: int, source_hwnd=None):
        ...
        if not pool:
            return
        # source_hwnd : fourni par le mode déplacement (clic souris sur une
        # fenêtre Dofus). Sans ça, Windows a déjà mis cette fenêtre au
        # premier plan avant l'appel — utiliser foreground_handle() ici
        # désignerait alors la fenêtre cliquée comme "actuelle", cassant le
        # calcul du personnage suivant. Avec source_hwnd, on sait quelle
        # fenêtre a réellement été cliquée et on calcule le suivant à partir
        # d'elle.
        current_hwnd = source_hwnd if source_hwnd is not None else foreground_handle()
        ...
        self._focus_guard.set_last_active(chosen.hwnd)
    ...
    def go_main(self):
        if not self._main or self._main in self._excluded:
            return
        for w in self._windows:
            if w.pseudo == self._main:
                activate_window(w.hwnd)
                self._focus_guard.set_last_active(w.hwnd)
